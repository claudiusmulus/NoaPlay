...
struct MemoryCardLevelDetails: Reducer {
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case .delegate:
                return .none
            case .goToNextLevelButtonTapped:
                if let nextLevel = state.completedLevel.nextLevel(difficulty: state.difficulty) {
                    return .run { send in
                        await send(
...
struct MemoryCardLevelDetailsView: View {
    let store: StoreOf<MemoryCardLevelDetails>
    ...
    private func verticalActionView(
        viewStore: ViewStoreOf<MemoryCardLevelDetails>
    ) -> some View {
        VStack(spacing: 20) {
...
#Preview {
    MemoryCardLevelDetailsView(
        store: .init(
            initialState: MemoryCardLevelDetails.State(
                completedLevel: .one,
                difficulty: .easy,
                gameDuration: .seconds(3600)
            )
        ){
            MemoryCardLevelDetails()
        }
    )
}
