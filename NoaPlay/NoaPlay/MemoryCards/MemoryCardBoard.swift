...
struct MemoryCardBoard: Reducer {
    struct State: Equatable {
        ...
        var showLevelDetails: MemoryCardLevelDetails.State?
    ...
    enum Action: Equatable {
        ...
        case showLevelDetails(MemoryCardLevelDetails.Action)
        case flipCard(id: MemoryCard.State.ID, value: String)
...
struct CardBoard: View {
    ...
    struct ViewState: Equatable {
        let cards: IdentifiedArrayOf<MemoryCard.State>
        let level: MemoryCardGame.Level
        let showTimer: Bool
        let gameDuration: String
        let didStartGame: Bool
        
        init(state: MemoryCardBoard.State) {
        ...
        NavigationStack {
            WithViewStore(self.store, observe: ViewState.init) { viewStore in
                ...
                .showDetails(
                    ...
                    MemoryCardLevelDetailsView(store: store)
                }
            }
        }
    }
